...
add_action( 'wp_footer', function() {
    ...
    foreach ( $product->get_available_variations() as $variation ) {
        if ( empty( $variation['is_in_stock'] ) ) {
            $attr_values = array_values( $variation['attributes'] ?? [] );
            $display_name = ! empty( $attr_values ) ? implode( ', ', array_filter( $attr_values ) ) : '';
            $oos_variations[] = [
...
(function($){
    $(document).ready(function(){
        ...
        var oosVariations = <?php echo wp_json_encode( $oos_variations ); ?>;
        ...
        function findMatchingOosVariationLabel() {
            var selected = getSelectedValues();

            if ( ! selected.length || ! oosVariations.length ) {
                return oosVariations.length ? oosVariations[0] : null;
            }

            for ( var i = 0; i < oosVariations.length; i++ ) {
                var item = oosVariations[i];
                if ( ! item.label ) continue;

                var parts = item.label.split(',').map(function(p){ return p.trim(); });

                var match = selected.every(function(sel){
                    return parts.indexOf(sel) !== -1;
                });
            ...
            return oosVariations[0] || null;
        }

        function showWatchdogForVariation( variationObj ) {
            if ( ! variationObj ) {
                hideWatchdog();
                return;
            }

            var label = variationObj.label ? variationObj.label : 'Tato varianta';

            if ( registered[ variationObj.id ] ) {
                $msg.html('Varianta &ldquo;' + label + '&rdquo; není skladem.').css('color','#c00').show();
                $question.hide();
                $formRow.hide();
            ...
            $vidInput.val( variationObj.id );
            $msg.html('Varianta &ldquo;' + label + '&rdquo; není skladem.').css('color','#c00').show();
            $question.text('Mám Tě informovat, až bude naskladněna?').show();
        ...
        function refreshWatchdog() {
            ...
            if ( ! oosVariations.length ) {
                hideWatchdog();
                return;
            }

            var matched = findMatchingOosVariationLabel();
            showWatchdogForVariation( matched );
        }
        ...
        $form.on('found_variation', function(e, variation){
            ...
            if ( ! variation.is_in_stock ) {
                ...
                var attrText = [];
                $form.find('.variations select').each(function(){
                    var text = $(this).find('option:selected').text() || $(this).val();
                    if ( text ) {
                        attrText.push( text.replace(/\s+–\s+není skladem$/i, '').trim() );
                    }
                });

                matched.label = attrText.join(', ');
                showWatchdogForVariation( matched );
            } else {
                refreshWatchdog();
            }
        ...
        $btn.on('click', function(){
            ...
            if ( ! $vidInput.val() ) {
                $resp.text('Vyber prosím variantu, kterou chceš hlídat.').css('color','red').show();
                return;
            }
