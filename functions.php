...
echo   '<div class="sp-hlidaci-pes-inline" data-product-id="' . esc_attr( $product_id ) . '" data-variation-id="0">';
echo     '<p class="sp-hlidaci-label">' . esc_html__( 'Mám Tě informovat, až bude produkt naskladněn?', 'hlidaci-pes' ) . '</p>';
echo     '<div class="sp-hlidaci-form hp-simple-form-row">';
echo       '<span class="sp-hlidaci-icon-wrap">' . $hp_svg_simple . '</span>';
echo       '<input type="email" class="sp-hlidaci-email" placeholder="' . esc_attr__( 'Tvůj e-mail', 'hlidaci-pes' ) . '">';
echo       '<input type="hidden" name="hp_nonce" value="' . esc_attr( $nonce ) . '">';
echo       '<button type="button" class="sp-hlidaci-btn custom-product-btn" title="' . esc_attr__( 'Upozorníme Tě mailem, jakmile bude zboží skladem.', 'hlidaci-pes' ) . '">' . esc_html__( 'Hlídací pes', 'hlidaci-pes' ) . '</button>';
echo     '</div>';
...
add_action( 'wp_footer', function() {
    ...
    $has_any_oos_variation = false;
    foreach ( $product->get_available_variations() as $variation ) {
        if ( empty( $variation['is_in_stock'] ) ) {
            $has_any_oos_variation = true;
            break;
        }
    }
    ?>
...
(function($){
    $(document).ready(function(){
        ...
        var hasAnyOosVariation = <?php echo $has_any_oos_variation ? 'true' : 'false'; ?>;
        ...
        function showGenericWatchdog() {
            if ( ! hasAnyOosVariation ) {
                hideWatchdog();
                return;
            }

            $msg.html('Některé varianty momentálně nejsou skladem.').css('color','#c00').show();
            $question.show();
        ...
        $form.on('woocommerce_variation_has_changed', function(){
            ...
            // Pokud není vše vybráno, ale produkt má OOS varianty, drž watchdog viditelný
            if ( hasEmpty ) {
                showGenericWatchdog();
            }
        });
        ...
        $form.on('found_variation', function(e, variation){
            ...
            if ( ! variation.is_in_stock ) {
                ...
                // Varianta skladem → pokud existují i jiné OOS varianty, nech watchdog dole stále
                showGenericWatchdog();
            }
        });
