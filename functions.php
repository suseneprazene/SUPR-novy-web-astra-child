...
add_filter( 'woocommerce_dropdown_variation_attribute_options_html', function( $html, $args ) {
    ...
    if ( ! $product || ! $product->is_type( 'variable' ) ) return $html;

    // Mapa: hodnota atributu → je alespoň jedna varianta skladem?
    // WooCommerce může klíče atributů vracet různě (s pa_, bez pa_, se sanitize_title),
    // proto zkoušíme více možných klíčů pro spolehlivé párování.
    $stock_map     = [];
    $attr_slug     = sanitize_title( $attribute );
    $attr_key_full = 'attribute_' . $attr_slug;
    $attr_key_pa   = 'attribute_pa_' . preg_replace( '/^pa_/', '', $attr_slug );

    foreach ( $product->get_available_variations() as $variation ) {
        // Zkus oba možné klíče
        if ( isset( $variation['attributes'][ $attr_key_full ] ) ) {
            $val = $variation['attributes'][ $attr_key_full ];
        } elseif ( isset( $variation['attributes'][ $attr_key_pa ] ) ) {
            $val = $variation['attributes'][ $attr_key_pa ];
        } else {
            // Fallback: projdi všechny klíče a najdi ten, který obsahuje slug atributu
            $val = null;
            foreach ( $variation['attributes'] as $k => $v ) {
                if ( false !== strpos( $k, $attr_slug ) ) {
                    $val = $v;
                    break;
                }
            }
        ...
        if ( $variation['is_in_stock'] ) {
            $stock_map[ $val ] = true;
        } elseif ( ! isset( $stock_map[ $val ] ) ) {
            $stock_map[ $val ] = false;
        }
    }

    // Přidáme disabled + suffix k out-of-stock <option>
    $html = preg_replace_callback(
        ...
        function( $m ) use ( $stock_map ) {
            ...
            if ( $value === '' ) return $m[0]; // placeholder

            $in_stock = $stock_map[ $value ] ?? true;

            if ( ! $in_stock ) {
                $label  .= ' – není skladem';
                $before  = ' data-stock="outofstock"' . $before;
                $after  .= ' disabled';
            } else {
                $before = ' data-stock="instock"' . $before;
            }
...
(function($){
    $(document).ready(function(){
        ...
        // Registrované variation_id (aby se formulář skryl po úspěchu)
        var registered = {};

        function showWatchdog( variationId, labelHtml ) {
            if ( registered[ variationId ] ) {
                // Už zaregistrováno – zobraz jen potvrzení, ne formulář
            ...
            $msg.html( labelHtml ).css('color','#c00').show();
            $question.show();
$formRow.css('display', 'flex');            // Reset stavu formuláře
            $resp.hide().text('');
            $email.show().val('');
        ...
        // ── Při načtení: zablokuj OOS options a ukaž hlídáčka pro první OOS variantu ──
        $form.find('.variations select option[data-stock="outofstock"]').each(function(){
            $(this).prop('disabled', true);
        });

        // Zjisti, jestli je aktuálně vybraná varianta OOS (nebo jestli existuje default OOS varianta)
        // WooCommerce nám po DOMContentReady pošle found_variation event jen pokud je default variant –
        // proto projdeme variace sami a zobrazíme hlídáčka pro ty OOS.
        var variationsData = $form.data('product_variations');
        if ( variationsData ) {
            // Najdi všechny OOS varianty
            var oosVariations = $.grep(variationsData, function(v){ return ! v.is_in_stock; });
            if ( oosVariations.length > 0 ) {
                // Pokud je jen jedna OOS, zobraz ji rovnou; jinak zobraz obecnou zprávu se seznamem
                if ( oosVariations.length === 1 ) {
                    var v = oosVariations[0];
                    var attrVals = [];
                    $.each(v.attributes, function(k, val){ if (val) attrVals.push(val); });
                    var label = 'Varianta &ldquo;' + attrVals.join(', ') + '&rdquo; není skladem.';
                    showWatchdog( v.variation_id, label );
                } else {
                    // Více OOS variant – zobraz obecnou zprávu, konkrétní varianta se doplní při výběru
                    var names = [];
                    $.each(oosVariations, function(i, v){
                        var attrVals = [];
                        $.each(v.attributes, function(k, val){ if (val) attrVals.push(val); });
                        names.push(attrVals.join(', '));
                    });
                    $msg.html('Varianty &ldquo;' + names.join('&rdquo;, &ldquo;') + '&rdquo; nejsou skladem.').css('color','#c00').show();
                    $question.show();
                    // Formulář neschováváme – ale variation_id zatím není nastaven;
                    // zákazník musí OOS vybrat (ale nemůže – jsou disabled), takže
                    // zobrazíme jen info text bez formuláře dokud WC nevyemituje found_variation
                }
            }
        }
        ...
        $form.on('found_variation', function(e, variation){
            if ( ! variation.is_in_stock ) {
                ...
                // Sestavíme label ze zvolených variant
                var attrText = [];
                $form.find('.variations select').each(function(){
                    var val = $(this).val();
                    if ( val ) attrText.push( val );
                });
